---
title: 'React 19 新功能：Action API、use 钩子与元数据管理'
...
React 19 Beta 在 2024 年释出，标志着 React 团队将此前的“并发基础设施”真正向开发者开放：Actions、`use` 钩子、Document Metadata、Asset Loading 等能力逐步走向稳定，并配合服务器组件（RSC）形成端到端的架构闭环。本文整理关键特性、适用场景与迁移注意事项，帮助团队提前规划升级路线。
...
- `useOptimistic`：在等待服务器响应期间应用乐观更新，提供撤销与合并策略，优化协作型应用的即时反馈。
...
React 19 将此前实验性的 `use` 钩子纳入正式 API。`use` 可以在组件中直接等待 Promise，读取 Context 或资源引用，并在 Suspense 边界内自动处理加载与错误。
...
React 19 允许在任意组件中直接声明文档元数据，React DOM 会对 `<title>`、`<meta>`、`<link>` 等标签进行去重和优先级合并。这让布局组件、页面组件、甚至嵌套的业务模块都能准确描述自己的 SEO 与分享信息。
...
React 19 提供 `ReactDOM.preload`、`ReactDOM.preconnect`、`ReactDOM.prefetchDNS` 等 API，允许在组件层面声明资源加载优先级。配合 Suspense，可以在组件渲染前预加载关键 CSS、字体或第三方脚本。
