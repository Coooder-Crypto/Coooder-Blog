...
- 优点：实现简单；原型方法可被所有实例共享，减少内存占用。
- 缺点：无法向父构造函数传参；父级引用类型属性会被所有子实例共享，容易造成数据串扰。

#### 借用构造函数继承（伪经典继承）

- 优点：可以在子类构造函数中向父类传参；实例之间不会共享父级引用类型属性。
- 缺点：父类方法无法复用，每个子实例都需要复制一份，内存开销大；原型上的方法无法访问。

#### 组合继承

- 优点：结合了原型链和借用构造函数的优势，既能传参又能复用原型上的方法。
- 缺点：父构造函数会被调用两次（一次在子构造函数内，一次在原型赋值时），会产生冗余属性。

#### 寄生式继承

- 优点：基于 `Object.create()` 快速构建对象，可灵活添加额外能力。
- 缺点：每次创建实例都要为新增方法创建新函数，无法复用；难以追踪构造过程。

#### 寄生组合式继承

- 优点：通过一次 `Object.create()` 建立原型链，只调用一次父构造函数；既支持传参又可复用方法，是传统继承的最佳实践。
- 缺点：实现写法相对繁琐，需要辅助函数来封装继承逻辑。

#### ES6 class 继承

- 优点：语法更接近传统面向对象语言，`super`/`extends` 等关键字让继承关系更直观；本质仍然基于原型链，与旧代码兼容。
- 缺点：语法糖可能掩盖原型链细节，调试时需要理解底层原理；某些高级场景下（如 mixin、动态继承）仍需回到原型层面操作。
...
function safeExtend(target, source) {
  const safeKeys = Object.keys(source).filter((key) => !['__proto__', 'constructor', 'prototype'].includes(key));
